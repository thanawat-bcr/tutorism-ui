...
class Solver {
  ...
  target: Color = new Color(0, 0, 0);
  targetHSL: {h: number, s: number, l: number} = {h: 0, s: 0, l: 0};
  reusedColor: Color = new Color(0, 0, 0);
...
export default function useColorToFilter(hexColor: string) {
  const rgb = hexToRgb(hexColor);
  const color = new Color(rgb[0], rgb[1], rgb[2]);
  const solver = new Solver(color);
  const solved = solver.solve();
  const result = ref(solved.filter);
